...
<main class="h-full overflow-y-auto">
    <div class="container px-4 mx-auto grid">
        ...
        ?>
            <a href="./task-history?col_care_call_Id=<?php echo $Mycare_call_Id;
                                                    } ?>" style="text-decoration: none;">
                <input type="submit" name="btnSelectQueryByDate" value="Task history" style="background-color:rgba(22, 160, 133,1.0); margin-top:-30px; margin-bottom:15px; color:white; border:none; border-radius:5px; cursor:pointer; font-size:15px; padding:8px; width:130px; height:55px;">
            </a>
            <br>
            <!-- Cards -->
            <div class="grid gap-6 mb-8 md:grid-cols-4 xl:grid-cols-4">
                ...
                if (isset($_GET['col_care_call_Id'])) {
                    ...
                    $result = mysqli_query($conn, "SELECT * FROM tbl_finished_meds WHERE col_care_call_Id='$Mycare_call_Id' ORDER BY med_Id DESC ");
                    while ($rowAreaDate = mysqli_fetch_array($result)) {
                        ...
                        echo "
                        <div style='margin-top: -30px; background-color:rgba(255, 234, 167,.1); box-shadow: rgba(100, 100, 111, 0.2) 0px 7px 29px 0px;' class='shadow-sm p-3 mb-5 bg-body flex mb-4 font-semibold text-gray-600 dark:text-gray-300 items-center dark:bg-gray-800'>
