            ...
            <div style="margin-top: -10px;" class="grid gap-6 mb-8 md:grid-cols-2">
                <div class="min-w-0 p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
                    ...
                    if (isset($_GET['col_taskId'])) {
                        ...
                        $sel_client_meds_result = mysqli_query($conn, "SELECT * FROM tbl_clients_medication_records WHERE col_taskId='$col_taskId' AND col_company_Id='" . $_SESSION['usr_compId'] . "' ");
                        while ($get_client_meds_row = mysqli_fetch_array($sel_client_meds_result)) { ?>

                            <div style="width:100%; height:auto; padding:2px;" class="my-6 font-semibold text-gray-700 dark:text-gray-200">
                                <?php echo "" . $get_client_meds_row['med_name'] . "<span style='color:rgba(39, 174, 96,1.0); font-size:14px;'> " . $get_client_meds_row['med_dosage'] . "</span>"; ?>
                            </div>

                            <div style="width:100%; height:auto; padding:2px; margin-top:-20px; color:rgba(44, 62, 80,.8);" class="my-6 font-semibold text-gray-700 dark:text-gray-200">
                                <?php echo "" . $get_client_meds_row['med_details'] . "" ?>
                            </div>
                            ...
                            <div style="margin-top: -20px;" class="flex mb-0 font-semibold text-gray-600 dark:text-gray-300 items-center p-1 bg-white dark:bg-gray-800">
                                <div style='width:100%; height:auto;display:block;'>
                                    ...
                                    <form action="./processing-med-submision?col_taskId=<?php echo $col_taskId ?>" method="POST" enctype="multipart/form-data" autocomplete="off">
                                        <input type="hidden" value="<?php echo "" . $get_client_meds_row['med_Id'] . ""; ?>" name="txtTask_IdNo" />
                                        <input type="hidden" value="<?php echo "" . $get_client_meds_row['col_occurence'] . ""; ?>" name="txtTask_Date" />
                                        <input type="hidden" value="<?php echo "" . $get_client_meds_row['med_name'] . ""; ?>" name="txtTaskName" />
                                        <input type="hidden" value="<?php echo "" . $get_client_meds_row['med_dosage'] . ""; ?>" name="txtDosage" />
                                        ...
                                        <input type="hidden" value="<?php echo "" . $get_client_meds_row['col_taskId'] . ""; ?>" name="txtTaskId" />
                                        <input type="hidden" value="<?php echo "" . $get_client_meds_row['uryyToeSS4'] . ""; ?>" name="txtClientId" />
                                        <input type="hidden" value="<?php echo "" . $get_client_meds_row['col_company_Id'] . "";
                                                                }
                                                            } ?>" name="txtClientCompId" />
                                        <?php
                                        $sel_carers_result = mysqli_query($conn, "SELECT * FROM tbl_goesoft_carers_account 
                                        WHERE user_email_address='" . $_SESSION['usr_email'] . "' AND col_company_Id='" . $_SESSION['usr_compId'] . "' ");
                                        while ($get_carers_data_row = mysqli_fetch_array($sel_carers_result)) { ?>

                                            <input type="hidden" value="<?php echo "" . $get_carers_data_row['user_fullname'] . "";
                                                                    } ?>" name="txtCarerName" />

                                            <?php
                                            $get_care_callId = mysqli_query($conn, "SELECT * FROM tbl_daily_shift_records 
                                            WHERE shift_date = '$today' AND col_carer_Id='" . $_SESSION['usr_carerId'] . "' 
                                            ...
                                            while ($careCall_row = mysqli_fetch_array($get_care_callId)) { ?>
                                                ...
                                                <hr>
                                                <h5 style="margin-top: 12px;">Medication status</h5>
                                                <br>

                                                <div class="form-group">
                                                    &nbsp; <input type="checkbox" value="Not taken" class="bedroomsCheck" id="displayPopupNoOption" name="txtcheckNObox" class="form-control" /> &nbsp; <label for="No">No / Not taken</label>
                                                </div>
                                                ...
                                                <div class="form-group">
                                                    &nbsp; <input type="checkbox" value="Fully taken" class="bedroomsCheck" id="displayPopupYesOption" name="txtcheckYesbox" class="form-control" /> &nbsp; <label for="Yes">Yes / fully taken</label>
                                                </div>

                                                <div id="popupYesOptions" style="width: 100%; height:auto; margin-top:5px;">
                                                    <label style="font-size: 16px; color:rgba(44, 62, 80,.7);" for="for note">E.g Prompt, observed, not observed...</label>
                                                    <br>
                                                    <br>
                                                    <textarea style="margin-top: -12px; resize:none;" name="txtTaskNoteYesOp" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-900 dark:bg-gray-700 form-textarea focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray" rows="3" placeholder="Note"></textarea>
                                                </div>
                                                <br>
                                                <button name="btnSubmitMedNote" type="submit" class="px-3 py-3 font-small leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                                                    Submit note
                                                </button>
                                    </form>
