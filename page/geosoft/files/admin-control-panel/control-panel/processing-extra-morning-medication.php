...
if (isset($_POST['btnSubmitExtraMorningMeds'])) {
    ...
    if ($countRes) {
      ...
      $(document).ready(function() {
        ...
        if ($count_Id_Res != 0) {
            ...
            if ($queryIns) {
                $updateMysqlTable1 = mysqli_query($conn, "UPDATE `tbl_clients_medication_records` SET `extra_call1`= '$txtCareCall', `med_colours`= '$med_colours', `visibility`= '$med_visibility' WHERE `client_med_Id` = '$myMedsIdentity' ");
                if ($updateMysqlTable1) {
                    header("Location: ./client-extra-morning-medication?uryyToeSS4=$txtClientSocialId");
                } else {
            ...
            if ($queryIns) {
                $updateMysqlTable1 = mysqli_query($conn, "UPDATE `tbl_clients_medication_records` SET `extra_call1`= '$txtCareCall', `med_colours`= '$med_colours', `visibility`= '$med_visibility' WHERE `client_med_Id` = '$myMedskId' ");
                if ($updateMysqlTable1) {
                    header("Location: ./client-extra-morning-medication?uryyToeSS4=$txtClientSocialId");
                } else {
                    header("Location: ./not-successful-page");
