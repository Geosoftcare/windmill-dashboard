...
if (isset($_POST['btnExtraBedTask'])) {
    ...
    if ($countRes) {
      ...
      $(document).ready(function() {
        ...
        if ($count_Id_Res != 0) {
            ...
            if ($queryIns) {
                $updateMysqlTable1 = mysqli_query($conn, "UPDATE `tbl_clients_task_records` SET `extra_call4`= '$txtCareCall', `task_colours`= '$task_colours', `visibility`= '$task_visibility' WHERE `client_task_Id` = '$myTaskIdentity' ");
                if ($updateMysqlTable1) {
                    header("Location: ./client-extra-bed-tasks?uryyToeSS4=$txtClientSocialId");
                } else {
            ...
            if ($queryIns) {
                $updateMysqlTable1 = mysqli_query($conn, "UPDATE `tbl_clients_task_records` SET `extra_call4`= '$txtCareCall', `task_colours`= '$task_colours', `visibility`= '$task_visibility' WHERE `client_task_Id` = '$client_task_id' ");
                if ($updateMysqlTable1) {
                    header("Location: ./client-extra-bed-tasks?uryyToeSS4=$txtClientSocialId");
                } else {
                    header("Location: ./not-successful-page");
