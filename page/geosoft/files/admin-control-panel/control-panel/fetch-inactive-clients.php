...
if ($varGetAllData == $varCookieCity) {
    ...
    if (isset($_POST["query"])) {
        ...
        ON 
            ...
            AND t2.col_end_date = 'TFN')) WHERE (t1.col_company_Id = '" . $_SESSION['usr_compId'] . "' AND t2.col_end_date = 'TFN' 
            AND t2.col_reason IN ('Deceased', 'Left Service', 'Hospitalized', 'Inactive', 'Holiday', 'With family', 'Permanent', 'Other') 
            AND t1.client_first_name LIKE '%" . $search . "%' OR t1.client_last_name LIKE '%" . $search . "%') GROUP BY t1.client_first_name";
    } else {
        $query = "
    ...
    'Left Service', 'Hospitalized', 'Inactive', 'Holiday', 'With family', 'Permanent', 'Other')) GROUP BY t1.client_first_name ASC;
";
    }
    ...
    if (isset($_POST["query"])) {
        ...
        ON 
            ...
            AND t2.col_end_date = 'TFN' AND t2.col_reason IN ('Deceased', 'Left Service', 'Hospitalized', 'Inactive', 'Holiday', 'With family', 'Permanent', 'Other') 
            AND t1.client_first_name LIKE '%" . $search . "%' OR t1.client_last_name LIKE '%" . $search . "%') GROUP BY t1.client_first_name";
    } else {
        $query = "
    SELECT t1.userId, t1.client_first_name, t1.client_last_name, t1.client_primary_phone, 
    ...
    LEFT JOIN tbl_client_status_records t2 
    ON t1.uryyToeSS4 = t2.col_client_Id 
    WHERE (t1.col_Office_Incharge = '" . $varCookieCity . "' AND t1.col_company_Id = '" . $_SESSION['usr_compId'] . "' AND t2.col_end_date = 'TFN' 
    AND t2.col_reason IN ('Deceased', 'Left Service', 'Hospitalized', 'Inactive', 'Holiday', 'With family', 'Permanent', 'Other')) 
    GROUP BY t1.client_first_name ASC;
";
    }
    $result = mysqli_query($conn, $query);
    if (mysqli_num_rows($result) > 0) {
