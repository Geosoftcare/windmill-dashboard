...
<div class="card table-card">
    <div class="card-header">
        ...
        if (isset($_GET['uryyToeSS4'])) {
            ...
            $result = mysqli_query($conn, "SELECT * FROM tbl_clients_task_records WHERE uryyToeSS4='$uryyToeSS4' AND col_company_Id = '" . $_SESSION['usr_compId'] . "' ORDER BY client_Id DESC LIMIT 1");
            while ($row = mysqli_fetch_array($result)) {
                echo "
                                                                <a href='./client-medication?uryyToeSS4=" . $row['uryyToeSS4'] . "' style='position: absolute; right:50px; top:6px;'>
        ...
        <div class="table-responsive">
            <table class="table table-striped table-hover mb-0">
                ...
                <tbody>
                    ...
                    if (isset($_GET['uryyToeSS4'])) {
                        $uryyToeSS4 = $_GET['uryyToeSS4'];
                        $result = mysqli_query($conn, "SELECT client_taskName,client_Id,col_period_two,col_period_one,col_extra_visit, SUBSTRING(monday, 1, 1) AS MDay, SUBSTRING(tuesday, 1, 1) AS TDay, SUBSTRING(wednesday, 1, 1) AS WDay, SUBSTRING(thursday, 1, 1) AS THDay, SUBSTRING(friday, 1, 1) AS FDay, SUBSTRING(saturday, 1, 1) AS SDay, SUBSTRING(sunday, 1, 1) AS SATDay, 
                        ...
                        FROM tbl_clients_task_records WHERE (uryyToeSS4='$uryyToeSS4' AND col_company_Id = '" . $_SESSION['usr_compId'] . "') ORDER BY client_Id ASC");
                        while ($row = mysqli_fetch_array($result)) {
                            echo "
                                                                                <tr>
                                                                                    <td>
