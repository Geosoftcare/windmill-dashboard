<?php
$mysqli = mysqli_connect('localhost', 'root', '', 'geosoft');
$total_pages = $mysqli->query('SELECT * FROM tbl_general_team_form')->num_rows;
$page = isset($_GET['page']) && is_numeric($_GET['page']) ? $_GET['page'] : 1;
$num_results_on_page = 5;

if ($stmt = $mysqli->prepare('SELECT * FROM tbl_general_team_form ORDER BY team_first_name LIMIT ?,?')) {
    ...
    <body>
        <table>
            ...
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    <td><?php echo $row['team_first_name']; ?></td>
                    <td><?php echo $row['team_date_of_birth']; ?></td>
                    <td><?php echo $row['team_nationality']; ?></td>
                </tr>
            <?php endwhile; ?>
        </table>
        <?php if (ceil($total_pages / $num_results_on_page) > 0): ?>
            <ul class="pagination">
                ...
                <li class="currentpage"><a href="pagination?page=<?php echo $page ?>"><?php echo $page ?></a></li>
                ...
                <?php if ($page < ceil($total_pages / $num_results_on_page) - 2): ?>
                    <li class="dots">...</li>
                    <li class="end"><a href="pagination?page=<?php echo ceil($total_pages / $num_results_on_page) ?>"><?php echo ceil($total_pages / $num_results_on_page) ?></a></li>
                <?php endif; ?>

                <?php if ($page < ceil($total_pages / $num_results_on_page)): ?>
                    <li class="next"><a href="pagination?page=<?php echo $page + 1 ?>">Next</a></li>
                <?php endif; ?>
            </ul>
        <?php endif; ?>
    </body>
