                ...
                <div class="row align-items-center">
                    <div class="col-md-12">
                        <div class="page-header-title">
                            <h4 class="m-b-10">Planned visits
                                <br>
                                <small>
                                    Review, planned visits before they're added to timesheets and/or invoices
                                </small>
                            </h4>
                        </div>
                    </div>
            ...
            <div style="padding: 0;" class="col-md-3">
                <div class="form-cover" style="box-shadow: rgba(100, 100, 111, 0.2) 0px 7px 29px 0px; padding:25px 15px 25px 15px; border-radius:12px;">
                    <form action="./set-dashboard-cookie" method="post" enctype="multipart/form-data" autocomplete="off" name="formData">
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <input type="submit" name="btnFetchPlannedVisitData" class="btn btn-small btn-info" value="Fetch data" />
                            </div>
                        </div>
                    </form>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                $result_count = mysqli_query($conn, "SELECT COUNT(*) As total_records FROM tbl_daily_shift_records WHERE ((shift_date >= '$txtStartDate') AND (shift_date <= '$txtEndDate') AND (uryyToeSS4 = '$txtCareRecipient' OR shift_status = '$txtCareRecipient') AND (col_carer_Id = '$txtCareGiver' OR shift_status = '$txtCareGiver') AND `col_company_Id` = '" . $_SESSION['usr_compId'] . "') ");
                                $total_records = mysqli_fetch_array($result_count);
                                $total_records = $total_records['total_records'];
                                $total_no_of_pages = ceil($total_records / $total_records_per_page);
                                $second_last = $total_no_of_pages - 1; // total page minus 1

                                $result = mysqli_query($conn, "SELECT * FROM tbl_daily_shift_records WHERE ((shift_date >= '$txtStartDate') AND (shift_date <= '$txtEndDate') AND (uryyToeSS4 = '$txtCareRecipient' OR shift_status = '$txtCareRecipient') AND (col_carer_Id = '$txtCareGiver' OR shift_status = '$txtCareGiver') AND `col_company_Id` = '" . $_SESSION['usr_compId'] . "') GROUP BY carer_Name LIMIT $offset, $total_records_per_page");
