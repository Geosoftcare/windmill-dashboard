...
if (isset($_POST['btnReScheduleRuns'])) {
    ...
    while ($row_carerId = mysqli_fetch_array($sel_carer_Id)) {
        ...
        while ($row = mysqli_fetch_array($sel_carer_Name)) {
            ...
            $sel_carer_Name2 = mysqli_query($conn, "SELECT userId FROM tbl_schedule_calls WHERE userId = '$db_userId' AND care_calls = 'Morning'");
            while ($row2 = mysqli_fetch_array($sel_carer_Name2)) {
                ...
                if ($sqlEditCareCall) {
                    ...
                    echo "Could not able to exceute" . mysqli_error($conn);
                }
            }
        }
    }
