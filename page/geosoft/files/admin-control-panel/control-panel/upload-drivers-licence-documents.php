...
if (isset($_POST['btnUpdateDocInfo'])) {
    ...
    if (is_uploaded_file($_FILES['file']['tmp_name']) and copy($_FILES['file']['tmp_name'], $target)) {
        $insert_queryIns = mysqli_query($conn, "UPDATE tbl_team_documents SET `col_drivers_licence_image` = '$txtDocument' WHERE uryyTteamoeSS4 = '$txtcompanyTeamId' AND col_company_Id = '$txtCompanyId' ");
        if ($insert_queryIns) {
            ...
            <div class="row">
                <div class="col-md-4">
                    ...
                    ?>
                        <div class="form-cover" style="box-shadow: rgba(100, 100, 111, 0.2) 0px 7px 29px 0px; padding:25px 15px 25px 15px; border-radius:12px;">
                            <form action="./upload-drivers-licence-documents?uryyTteamoeSS4=<?php echo $uryyTteamoeSS4; ?>" method="post" enctype="multipart/form-data" autocomplete="off">
                                ...
                                <div class="row">
                                    <div class="col-lg-12 col-12">
                                        <div class="form-group">
                                            <label for="Id Card/Passport">Driver's licence</label>
                                            <input type="text" value="<?php echo "" . $get_team_row['col_drivers_licence_image'] . "" ?>" class="form-control" placeholder="Social number" />
                                            <div>
                                                <label for="files" class="btn btn-dark btn-sm">Upload file</label>
                                                <input id="files" multiple onchange="GetFileSizeNameAndType()" value="<?php echo "" . $get_team_row['col_drivers_licence_image'] . "" ?>" name="file" style="visibility:hidden;" type="file">
                                            </div>
                                        </div>
                ...
                <div class="col-md-8">
                    <div class="form-cover" style="font-weight:800; box-shadow: rgba(100, 100, 111, 0.2) 0px 7px 29px 0px; padding:25px 15px 25px 15px; border-radius:12px;">
                        ...
                        <img style="width: 100%; height:auto;" src="./team_documents/<?php echo "" . $get_team_row['col_drivers_licence_image'] . "" ?>" alt="">
                    <?php
                    }
                    ?>
                    </div>
