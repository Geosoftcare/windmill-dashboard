...
if (isset($_POST['btnSendChat'])) {
    ...
    if ($countRes) {
        ...
        $queryIns = mysqli_query($conn, "INSERT INTO tbl_chat_system (carer_email, admin_email, admin_name, admin_specialId, admin_chat, adTime_sent, adDate_sent, adChat_color, col_company_Id) 
        VALUES('" . $txtCarerEmail . "', '" . $txtAdminEmail . "', '" . $txtAdminName . "', '" . $txtAdminSpecialId . "', '" . $txtChatMessage . "', '" . $txtAdminTimeSent . "', '" . $txtAdminDateSent . "', '" . $txtChatColor . "', '" . $_SESSION['usr_email'] . "') ");
        if ($queryIns) {
            header("Location: ./chat-system?user_special_Id=$txtCarerSpecialId");
        }
    }
