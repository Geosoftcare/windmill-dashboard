...
if (isset($_POST['btnCancelCall'])) {
    ...
    while ($row_carerId = mysqli_fetch_array($sel_carer_Id)) {
        ...
        while ($row = mysqli_fetch_array($sel_carer_Name)) {
            ...
            $sel_carer_Name2 = mysqli_query($conn, "SELECT userId,client_name,first_carer,second_carer FROM tbl_schedule_calls WHERE userId = '$db_userId' AND care_calls = 'Tea'");
            while ($row2 = mysqli_fetch_array($sel_carer_Name2)) {
                ...
                if ($sqlEditCareCall) {
                    ...
                    if ($queryIns) {
                        header("Location: ./edit-tea-call?uryyToeSS4=$txtClientSpecialId");
                    }
                } else {
                }
            }
