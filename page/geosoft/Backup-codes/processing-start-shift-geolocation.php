...
if (isset($_POST['btnStartShift'])) {
    ...
    $curdistance = ROUND(haversineGreatCircleDistance($deviceLat, $deviceLon, $postcodeLat, $postcodeLon));
    ...
    ////////////////////////////
    $sql_get_recent_visit_pc = mysqli_query($conn, "SELECT * FROM tbl_daily_shift_records WHERE (shift_date = '$today' 
    AND col_carer_Id = '$txtCarerId' AND col_company_Id = '" . $_SESSION['usr_compId'] . "') ORDER BY userId DESC LIMIT 1");
    ...
    if ($count_get_recent_visit_pc != 0) {
        ...
        if ($txtStartDate == $today) {
            if ($carecallStartTime > $fsDeviceTime) {
                ...
                $sql_check_today_visit = mysqli_query($conn, "SELECT * FROM tbl_daily_shift_records WHERE (shift_date = '$today' 
                AND col_carer_Id = '$txtCarerId' AND uryyToeSS4 = '$txtClientId' AND col_care_call = '$txtVariousCareCalls' 
                ...
                if ($count_check_today_visit != 0) {
                    ...
                    $_SESSION['uryyToeSS4'] = $txtClientId;
                    $_SESSION['clientcarecalluserIdVar'] = $txtClientSpecialUserId;
                    $sql_check_call_status = mysqli_query($conn, "SELECT * FROM tbl_daily_shift_records WHERE (shift_date = '$today' 
