...
while ($att_rw = mysqli_fetch_array($sel_dist_att)) {
  ...
  FROM `tbl_schedule_calls` WHERE (first_carer_Id = '$worker_specialId' AND rightTo_display = 'True' AND Clientshift_Date = '$today' 
...
?>
  ...
  <main class="h-full overflow-y-auto">
    <div class="container px-4 mx-auto grid">
      ...
      $query = "
    SELECT 
        c.userId, c.first_carer_Id, c.client_name, c.Clientshift_Date, 
        c.col_occurence, c.dateTime_in, c.dateTime_out, c.call_status, 
        c.bgChange, c.rightTo_display, c.col_required_carers, c.uryyToeSS4 
    ...
    WHERE 
        (c.Clientshift_Date = ? OR c.col_occurence = ?) 
    ...
    ORDER BY c.dateTime_in ASC";
      ...
      mysqli_stmt_bind_param($stmt, "ssss", $today, $today, $worker_specialId, $worker_companyId);
      ...
      if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
          ...
          $userId = isset($row['userId']) ? htmlspecialchars($row['userId']) : "Unknown";
          $clientName = isset($row['client_name']) ? htmlspecialchars($row['client_name']) : "No Name";
          $dateTimeIn = isset($row['dateTime_in']) ? htmlspecialchars($row['dateTime_in']) : "N/A";
          $dateTimeOut = isset($row['dateTime_out']) ? htmlspecialchars($row['dateTime_out']) : "N/A";
          $callStatus = isset($row['call_status']) ? htmlspecialchars($row['call_status']) : "Pending";
          $bgChange = isset($row['bgChange']) ? htmlspecialchars($row['bgChange']) : "#ccc"; // Default to grey if missing

          echo sprintf(
            '<div style="box-shadow: rgba(100, 100, 111, 0.2) 0px 7px 29px 0px; border-top-right-radius:15px; border-top:4px solid %s;" 
                class="shadow-sm p-3 bg-body flex mb-0 font-semibold text-gray-600 dark:text-gray-300 items-center dark:bg-gray-800">
                <a class="inline-flex items-center w-full text-sm font-semibold text-gray-800 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 dark:text-gray-100" href="./checking-visit-status?userId=%s">
                    ...
                    <div style="width:100%%;">
                        ...
                        <p style="font-size:14px;" class="text-sm font-semibold text-gray-700 dark:text-gray-200">
                            <span> %s - %s </span>
                            <br>
                            <span style="float:right; background-color:%s; color:white; padding:3px 16px 3px 16px; border-radius:50px; font-size:12px;">%s</span>
                        </p>
                    </div>
                </a>
            </div>',
