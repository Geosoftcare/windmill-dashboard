...
if (isset($_POST['btnOfficiallyCert'])) {
    ...
    $sql = mysqli_query($conn, "UPDATE `tbl_goesoft_carers_account` SET `care_plan_status` = '$txtOfficiallyCert' WHERE user_email_address = '$txtCarerEmail2' AND col_company_Id = '" . $_SESSION['usr_compId'] . "' ");
    if ($sql) {
        header("Location: ./care-plan?userId=$txtClientId2");
    }
}
